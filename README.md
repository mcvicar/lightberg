...
* Post the JSON to AWS API Gateway (see example_events/api_example.json)
...
* AWS API Gateway to a lambda function that parses the JSON and kicks off a fanout
* Fanout generates both HTML and JSON reports into different (if you like) S3 buckets
  * The Fanout lambda function is basically just https://github.com/joytocode/lighthouse-lambda
  * Each URL report filename is based on uuid v5 URL domain, so there shouldn't be clashes
...
## TODO
* Get SNS fanout, the summary step function and API gateway into terraform
* Add in AWS Quicksight into Terraform and create a default dashboard based on the JSON reports so you can track over time, etc.
